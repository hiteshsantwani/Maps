...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		 ...
		 int n = this.getNumVertices()*2;
	     int[][] tempMatrix = new int[n][n];
	     int rowsInA = adjMatrix.length;
	     int columnsInA = adjMatrix.length; 
	     int columnsInB = adjMatrix.length;
//	     System.out.println(" "+tempMatrix.length);
...
//	          	     System.out.println(" "+k);
	            	   tempMatrix[i][j] = tempMatrix[i][j] + adjMatrix[i][k] * adjMatrix[k][j];
...
//	    	 for (int j = 0; j < columnsInB; j++){
//	    		 if(tempMatrix[i][j]>1){
//	    			 for(int k = 0; k<tempMatrix[i][j]; k++){
//	    				 Distance2.add(tempMatrix[i][j]);
//	    			 }
//	    		 }
//	    	 }
//	     }
	     for (int i = 0; i < columnsInB; i++) {
	    	 if(tempMatrix[v][i]>0){
    			 for(int k = 0; k<tempMatrix[v][i]; k++){
    				 Distance2.add(i);
    			 }
    		 }
	     }
